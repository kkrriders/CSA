"""
ML prediction service with graceful fallback for missing sklearn.

This version works without scikit-learn for production deployments
that don't include heavy ML dependencies.
"""
...
class MLPredictionService:
    """Service for ML-based predictions (fallback version without sklearn)."""
    ...
    async def predict_success(
        ...
        if not STATS_AVAILABLE:
            return {
                ...
                "message": "Statistics library not available"
            }
    ...
    async def recommend_next_difficulty(
        ...
        for diff, scores in difficulty_scores.items():
            if scores:
                success_rates[diff] = sum(scores) / len(scores)
